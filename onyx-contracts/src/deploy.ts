/**
 * Deploy onyx-contracts contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
const createInitialPrivateState = (secretKey: Uint8Array): OnyxPrivateState => ({
  secretKey,
  listingSalts: {},
});
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      deployed = await deployContract(providers, {
        ...
        initialPrivateState: createInitialPrivateState(randomBytes(32)),
      });
      break;
    } catch (err: any) {
